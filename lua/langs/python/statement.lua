-- lua/langs/python/statement.lua

-- Completes a Python block header: if the current line looks like it
-- should end in a colon (an if/for/def/class/... header, or a line
-- treesitter says is already inside a block), adds the ':' if it's
-- missing and opens a new, correctly indented line below in insert
-- mode. Moved as-is from the old lua/utils/python.lua.
